// 这个版本是提取游民星空游戏页面pc发售表的数据, 并存储到MySQL数据库中
// 2025年02月08日11:38:57
// 修复了服务器上puppeteer的问题, 重新部署 (完美版)
var cheerio = require("cheerio");
const puppeteer = require("puppeteer");
const mysql = require("mysql2/promise");
var targetgametype = "pc";
var nowTime = "";
...
async function getHtml() {
  ...
  // url = `https://ku.gamersky.com/release/pc_202501/`;
  console.log("url :>> ", url);
...
async function saveDataToMySQL(dataArray) {
  ...
  try {
    ...
    for (let i = 0; i < dataArray.length; i++) {
      ...
      await connection.execute("delete from pc_game where title = ?", [dataArray[i].title]);
      await connection.execute(
        "insert into pc_game(title, url, img, time, game_type, production, introduction, update_time,targetgametype,player_rating,player_num,expected_value) values(?, ?, ?, ?, ?, ?, ?,?,?,?,?,?)",
        [
          dataArray[i].title,
          dataArray[i].url,
          dataArray[i].img,
...
// nodeSchedule.scheduleJob("50 30 11 * * *", async () => {
//   try {
//     await getHtml();
//   } catch (error) {
