	...
	<div class="login-container">
		...
		<div class="login-plane">
			<div class="login-plane-container">
				<img class="login-plane-human" src="@/assets/images/login_human.png" alt="宇航员" />
			</div>
		</div>
...
export default defineComponent({
	setup() {
		// 初始化组件
		const IMAGES_SKY = new URL("../../assets/images/sky.png", import.meta.url).href;
		const IMAGES_EARTHBG = new URL("../../assets/images/earth_bg.png", import.meta.url).href;
		const IMAGES_CLOUD = new URL("../../assets/images/cloud.png", import.meta.url).href;
		const IMAGES_STARFLAKE1 = new URL("../../assets/images/starflake1.png", import.meta.url).href;
		const IMAGES_STARFLAKE2 = new URL("../../assets/images/starflake2.png", import.meta.url).href;
		// 容器
		let container: any;
		// 声明视口宽度
		let width: number;
		...
		const initTubeRoute = (route?: any, geometryWidth?: number, geometryHeigh?: number) => {
			...
			const cloudTexture = textureLoader.load(IMAGES_CLOUD);
		...
		const initSceneStar = (initZposition: number): any => {
			...
			const textureLoader = new THREE.TextureLoader();
			const sprite1 = textureLoader.load(IMAGES_STARFLAKE1);
			const sprite2 = textureLoader.load(IMAGES_STARFLAKE2);
			parameters = [
		...
		// 初始化云的运动函数
		const initCloudMove = (cloudParameter: any, speed: number, scaleSpeed = 1, maxScale = 1, startScale = 0) => {
			let cloudProgress = 0;
			return () => {
